...
void main() {
  ...
  group('TC-1.7.1 不含 stack trace', () {
    test('FormatException 不暴露 stack', () {
      final info = classify(const FormatException('bad data'));
  ...
  group('TC-1.7.3 不含 LaTeX 源', () {
    test('userMessage 不含 \\frac / \\sum 等 LaTeX 命令', () {
      final cases = <Object>[
        const FormatException(r'\frac{1}{2} invalid'),
    ...
    test('userMessage 不含 \$E=mc^2\$ 等公式片段', () {
      final info = classify(const FormatException(r'E=mc^2 malformed'));
  ...
  group('TC-1.7.6 根因区分', () {
    ...
    test('timeout 与 offline 消息不同', () {
      ...
      final offlineInfo = classify(const SocketException('refused'));
    ...
    test('writeError 与 parseError 消息不同', () {
      final writeInfo = classify(const FileSystemException('disk full'));
      final parseInfo = classify(const FormatException('bad'));
