/// EditorCommand sealed class + 子类（用户意图，纯数据）。
///
...
///
/// **为什么 sealed class 必须与子类同文件**：
/// Dart sealed class 限定所有直接子类必须位于同一 library。本文件将 [EditorCommand]
/// 与所有 8 个子类合并到同一文件，外部仅通过 `export` 引用（见 commands.dart）。
///
...
///
/// **sealed 限定**：所有直接子类必须位于本文件（同 library）。外部 import 通过
/// [commands.dart] 的 `export 'editor_command.dart'` 引用。
@immutable
...
// ============ 8 种 EditorCommand 子类（sealed library 限定） ============
